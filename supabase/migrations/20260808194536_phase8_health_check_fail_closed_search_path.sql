-- Phase 8 Closure: Fix health check RPC to be fail-closed on SECURITY DEFINER/search_path/RLS
...
BEGIN
  ...
  -- RLS: every required table that exists must have RLS enabled (fail-closed)
  FOREACH v_tbl IN ARRAY v_tables LOOP
  ...
  -- Fail-closed: every SECURITY DEFINER function must have search_path set
  -- If any SECURITY DEFINER function lacks search_path, flag as unsafe
  ...
  RETURN jsonb_build_object(
    ...
    'security_definer', jsonb_build_object(
      'search_path_empty', v_search_path_ok,
      'total_count', v_secdef_count,
      'with_search_path_count', v_secdef_with_search_path_count
    ),
